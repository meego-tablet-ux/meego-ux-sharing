...
#ifndef SHARINGDAEMON_H
#define SHARINGDAEMON_H
...
class SharingServicePluginInterface;

class SharingDaemon : public QObject
{
    Q_OBJECT
public:
    explicit SharingDaemon(QObject *parent = 0);
    ~SharingDaemon();
    bool init();
...
private:
    ...
    QList<SharingServiceInfo> mServices;  //1 Plugin can provide > 1 service, and each service can service >= 1 type
    QHash<QString, SharingServicePluginInterface *> mServiceToPlugin;
