...
#include <sharing-daemon_adaptor.h>
...
#define DBUSOBJPATH QString("/com/intel/sharing")


SharingDaemon::SharingDaemon(QObject *parent) :
    QObject(parent),
    mGlobalOpID(0),
    mClosing(false)
{
...
bool SharingDaemon::init()
{
    //Load plugins
    return loadPlugins();
}

int SharingDaemon::Share(const QString &service, const QString &sharetype, ArrayOfShareItemStruct items, QString &errmessage)
{
    //TODO-JEA
    SharingServicePluginInterface *intf = mServiceToPlugin.value(service);
    if (!intf) {
...
bool SharingDaemon::CancelShare(int opid)
{
    ...
    SharingServicePluginInterface *intf = mServiceToPlugin.value(svcName);
    if (!intf) {
...
//Public slots:
QStringList SharingDaemon::GetAllServices()
{
    QStringList services;
    foreach (SharingServiceInfo info, mServices) {
        services << info.serviceName;
    }
    return services;
}

QStringList SharingDaemon::GetServicesForType(const QString &type)
{
    QStringList services;
    foreach(SharingServiceInfo info, mServices) {
...
void SharingDaemon::RescanPlugins()
{
...
void SharingDaemon::onSvcShareProgress(const QString &service, int opid,
...
bool SharingDaemon::loadPlugins()
{
    ...
    if (mClosing || !mPluginMutex.tryLock(3000))
        return false;
    QDir dir = QDir(SHARINGD_PLUGIN_PATH);
    ...
    foreach (QString file, dir.entryList(QStringList() << QString("*.so"))) {
        ...
        SharingServicePluginInterface *intf = qobject_cast<SharingServicePluginInterface *>(plugin->instance());
        if (!intf) {
        ...
        QString name = props.value(SHARING_SERVICE_PLUGIN_NAME, QString());
        if (name.isEmpty()) {
        ...
        foreach(SharingServiceInfo info, intf->getServices()) {
...
bool SharingDaemon::unloadPlugins()
{
    mPluginMutex.lock();
    ...
    while (it != mPlugins.end()) {
        ...
        if (plugin) {
            SharingServicePluginInterface *intf =
                    qobject_cast<SharingServicePluginInterface *>(plugin->instance());
            if (!intf) {
                qDebug() << QString("Ahh! Couldn't get an SSPI from plugin %1 on unload! Abort, abort!").arg(it.key());
            } else {
                foreach(SharingServiceInfo info, intf->getServices()) {
                    foreach(QString type, info.serviceTypes) {
